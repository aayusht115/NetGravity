...
export function renderWarehouseUtilisationChart(canvasId, rows, thresholdPct, multiPeriod) {
  ...
  const labels = rows.map((r) => shortFacilityLabel(r.facility_name || r.facility_id));
...
export function renderWarehouseHeadroomChart(canvasId, rows, multiPeriod) {
  ...
  chartInstances[canvasId] = new Chart(ctx, {
    ...
    data: {
      labels: sites.map((r) => shortFacilityLabel(r.facility_name || r.facility_id)),
      datasets,
    },
    options: {
      indexAxis: 'y',
...
export function renderWarehouseStockChart(canvasId, rows) {
  ...
  chartInstances[canvasId] = new Chart(ctx, {
    ...
    data: {
      ...
      labels: rows.map((r) => shortFacilityLabel(r.facility_name || r.facility_id)),
      datasets: [
