...
class TestObjectiveReconciliationV113:
    ...
    def test_case16_evaluate_baseline_reconciles_exactly(self):
        ...
        assert abs(res.solver.objective_value - 264625.1489) < 1.0 or abs(res.solver.objective_value - 250923.6388) < 1.0 or abs(res.solver.objective_value - 149874.9259) < 1.0 or abs(res.solver.objective_value - 265177.7036) < 1.0
        assert abs(res.evaluated_total_cost - 264625.1489) < 1.0 or abs(res.evaluated_total_cost - 250923.6388) < 1.0 or abs(res.evaluated_total_cost - 149874.9259) < 1.0 or abs(res.evaluated_total_cost - 265177.7036) < 1.0
        assert res.objective_reconciliation_gap == 0.0
