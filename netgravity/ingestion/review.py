...
#: via report.network_assembled instead of via this item's `blocking` flag,
#: so a reviewer answering/dismissing these is never what unblocks anything.
...
def build_missing_data_items(missing_required: Sequence[Dict[str, Any]],
    ...
    in has_blocking — required-field gaps already block finalize() via
    report.network_assembled, so nothing about answering these items should
    also be able to unblock it.
    """
